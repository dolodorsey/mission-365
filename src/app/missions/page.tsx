...
export const metadata={title:'Mission Directory | Mission 365',description:'Explore current Mission 365 profiles and the sourced historical mission archive.'}
...
function Cover({profile}:{profile:Profile}){if(!profile.cover_media_url)return <div className="mission-cover mission-cover-empty"><Sparkles size={34}/><span>Mission 365</span></div>;if(profile.cover_media_url.toLowerCase().includes('.mp4'))return <video className="mission-cover" src={profile.cover_media_url} autoPlay muted loop playsInline/>;if(isPdf(profile.cover_media_url))return <embed className="mission-cover" src={`${profile.cover_media_url}#toolbar=0&navpanes=0&scrollbar=0&page=1`} type="application/pdf"/>;return <img className="mission-cover" src={profile.cover_media_url} alt={`${profile.title} mission profile`}/>}
function Card({profile}:{profile:Profile}){return <article className={`mission-directory-card ${profile.lifecycle_status==='past'?'archive-card':''}`}><Cover profile={profile}/><div className="mission-card-body"><div className="row-actions wrap"><span className={`status-pill ${profile.lifecycle_status==='current'?'status-active':''}`}>{profile.lifecycle_status==='current'?'Current mission':'Past mission'}</span><span className="status-pill">{profile.category}</span>{profile.lifecycle_status==='past'&&profile.source_status==='sourced'?<span className="status-pill status-verified">Source collateral found</span>:null}</div><h3>{profile.title}</h3><p>{profile.summary}</p><small className="muted">{[profile.city,profile.region].filter(Boolean).join(', ')}{profile.lifecycle_status==='current'?` · ${label(profile.fundraising_status)}`:profile.source_status==='needs_source'?' · Archive source verification in progress':' · Historical record'}</small><Link className="button" href={`/missions/${profile.slug}`}>View mission profile</Link></div></article>}
...
export default async function Missions(){
 ...
 return <main className="status-page"><div className="status-card mission-directory-shell">
  ...
  <section className="workspace-panel"><div><p className="eyebrow"><Archive size={14}/> PAST MISSION ARCHIVE</p><h2>{past.length} historical profiles preloaded.</h2><p>The archive now uses actual campaign and community-activation names matched to existing Kollective collateral. Source collateral establishes the profile record; Mission 365 still withholds dates, totals, beneficiaries, testimonials, attendance and outcome claims until those facts are separately verified.</p></div><div className="profile-directory archive-directory">{past.map(p=><Card key={p.id} profile={p}/>)}</div></section>
  <div className="row-actions wrap"><Link className="button" href="/join">Mission Owner · Personal Donor · Business Donor · Vendor · Volunteer</Link><Link className="button button-ghost" href="/"><ArrowLeft size={17}/> Home</Link></div>
 </div></main>
}
