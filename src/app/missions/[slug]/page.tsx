...
import { Archive, BadgeCheck, CheckCircle2, Clock3, FileCheck2, HandHeart, Images, MessageSquareQuote, Store, Users } from 'lucide-react'
...
export default async function MissionPage({params}:Props){
 ...
 if(!profile)notFound()
 const [{data:media},{data:posts},{data:testimonials},{data:opportunities},{data:sources}]=await Promise.all([
  ...
  supabase.from('mission365_profile_sources').select('id,source_system,evidence_kind,verification_status,notes,created_at').eq('profile_id',profile.id).order('created_at',{ascending:true})
 ])
 ...
 return <main className="status-page"><div className="status-card mission-profile-shell">
  ...
  {profile.lifecycle_status==='past'&&profile.source_status==='sourced'&&<section className="workspace-panel notice-card"><p className="eyebrow">HISTORICAL SOURCE LOCATED</p><h2>The campaign or activation has source collateral in the Kollective vault.</h2><p>Mission 365 has matched this archive profile to existing source collateral. That confirms the named initiative has a source record; it does not automatically claim fundraising totals, attendance, beneficiaries, testimonials, dates, vendor payments, or outcomes. Those details stay unpublished until separately verified.</p></section>}
  ...
  {sources?.length?<section className="workspace-panel"><p className="eyebrow"><FileCheck2 size={14}/> SOURCE & PROVENANCE</p><h2>What this profile is grounded in.</h2><div className="record-list">{sources.map((s:any)=><div className="record-row" key={s.id}><div><strong>{s.source_system}</strong><small>{label(s.evidence_kind)} · {label(s.verification_status)}</small><p>{s.notes}</p></div></div>)}</div></section>:null}
