...
import { MISSION365_ADMIN_QUEUE_URL, MISSION365_LAUNCH_STATUS_URL, MISSION365_SUPABASE_PUBLISHABLE_KEY } from '../../../lib/mission365-public'
...
type LaunchStatus={
  ...
  qa:Array<{action:string;status:string|null;createdAt:string}>
}
type Candidate={
  id:string
  public_name:string
  ...
  actions:Array<{action:string;metadata:Record<string,unknown>;createdAt:string}>
}
type AdminQueue={
  role:string
  candidates:Candidate[]
  applications:Array<Record<string,unknown>>
  liveMissions:Array<Record<string,unknown>>
  payoutHolds:Array<Record<string,unknown>>
  riskEvents:Array<Record<string,unknown>>
  payments:{stripeApi:boolean}
}
  ...
  return <>
    ...
    <div className="role-grid" style={{marginTop:24}}>
      <article className="role-card">
        <p className="eyebrow">PAYMENT RELEASE GATE</p>
        <h3>{launch?.payments.liveGiving?'Live giving enabled':'Live giving credential-gated'}</h3>
        <p>Stripe API credential: <strong>{launch?.payments.stripeApi?'READY':'MISSING'}</strong><br/>Webhook signing: <strong>{launch?.payments.webhook?'READY':'NOT READY'}</strong></p>
        {launch?.payments.liveGiving?<ShieldCheck/>:<TriangleAlert/>}
      </article>
      <article className="role-card">
        <p className="eyebrow">QA EVIDENCE</p>
        <h3>Financial integrity checks</h3>
        {(launch?.qa||[]).length? <ul>{launch!.qa.map(row=><li key={`${row.action}-${row.createdAt}`}><strong>{formatAction(row.action)}</strong> — {row.status||'recorded'}</li>)}</ul>:<p>No production QA evidence has been recorded yet.</p>}
      </article>
    </div>

    <section style={{marginTop:28}}>
      <div style={{display:'flex',justifyContent:'space-between',gap:12,alignItems:'end',flexWrap:'wrap'}}>
        <div><p className="eyebrow">VERIFICATION COMMAND</p><h2 style={{marginTop:4}}>Launch Cohort & review queue</h2></div>
        {!signedIn&&<Link className="button" href="/login">Sign in as reviewer</Link>}
      </div>
      ...
      {queue?<div className="role-grid" style={{marginTop:16}}>
        {queue.candidates.map(candidate=><article className="role-card" key={candidate.id}>
          <p className="eyebrow">{candidate.verification_status.replaceAll('_',' ').toUpperCase()}</p>
          <h3>{candidate.public_name}</h3>
          <p>{candidate.organization_type.replaceAll('_',' ')}</p>
