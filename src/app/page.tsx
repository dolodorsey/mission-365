...
type HealthState='checking'|'healthy'|'degraded'

type BackendLinkProps={
  enabled:boolean
  href:string
  className?:string
  children:ReactNode
...
function BackendLink({enabled,href,className='',children}:BackendLinkProps){
  if(enabled) return <Link href={href} className={className}>{children}</Link>
  return <span className={className} aria-disabled="true" title="Temporarily unavailable" style={{opacity:.48,cursor:'not-allowed'}}>{children}</span>
}
...
export default function Home() {
  ...
  const givingLive=Boolean(backendAvailable&&health?.payments?.liveGiving&&health.liveMissions>0)

  return <main>
    <ApplicationNudge/>
    <nav className="nav-shell" aria-label="Primary navigation">
      <Link href="/" className="brand"><span className="brand-mark">M</span><span>MISSION <b>365</b></span></Link>
      <div className="nav-links"><a href="#model">How it works</a><BackendLink enabled={backendAvailable} href="/missions">Missions</BackendLink><BackendLink enabled={backendAvailable} href="/join">Join</BackendLink><BackendLink enabled={backendAvailable} href="/login">Sign in</BackendLink></div>
      <BackendLink enabled={backendAvailable} href="/apply" className="button button-small">Submit a mission <ArrowRight size={16}/></BackendLink>
    </nav>
    ...
    <section className="hero">
      <div className="hero-copy">
        ...
        <div className="hero-actions"><BackendLink enabled={backendAvailable} href="/missions" className="button">Explore missions <ArrowRight size={18}/></BackendLink><BackendLink enabled={backendAvailable} href="/join" className="button button-ghost">Choose how you participate</BackendLink></div>
        <div className="launch-state"><BadgeCheck/><div><strong>{backendBlocked?'Service temporarily unavailable':healthState==='checking'?'Checking service availability':givingLive?'Live giving active':'Verification-first launch'}</strong><span>{backendBlocked?'Backend-dependent actions remain paused until Mission 365 health is verified.':healthState==='checking'?'Mission 365 is verifying its dedicated service status before opening account and giving actions.':givingLive?'Verified missions are open for support.':'Public fundraising stays closed until missions complete verification and payout readiness.'}</span></div></div>
      </div>
    ...
    <section className="truth-strip"><div><strong>{backendBlocked?'OFFLINE':health?health.liveMissions:'—'}</strong><span>fundraising missions live</span></div><div><strong>{backendBlocked?'PAUSED':health?health.verificationCandidates:'—'}</strong><span>organizations in verification</span></div><div><strong>{backendBlocked?'HOLD':health?(health.payments.liveGiving?'READY':'HOLD'):'—'}</strong><span>payment rail status</span></div></section>
    ...
    <section className="partner-section"><div className="partner-art"><Image src="/brand/mission365-partner.png" alt="Mission 365 partnership artwork" fill sizes="(max-width:800px) 100vw,46vw"/></div><div className="partner-copy"><p className="eyebrow">Partners with purpose</p><h2>Turn community support into accountable action.</h2><p>Businesses can sponsor verified missions, support employee giving, and build a clear record of local impact. Service providers can also complete full network onboarding so approved mission needs can be matched with qualified vendors.</p><div className="row-actions wrap"><BackendLink enabled={backendAvailable} className="button" href="/join?role=donor_business">Join as a business <ArrowRight size={18}/></BackendLink><BackendLink enabled={backendAvailable} className="button button-ghost" href="/join?role=vendor">Join as a Mission 365 vendor <ArrowRight size={18}/></BackendLink><a className="button button-ghost" href={PROVIDER_ONBOARDING_URL}>Enterprise provider onboarding <ArrowRight size={18}/></a></div></div></section>

    <section className="final-cta"><p className="eyebrow">Give well. Report honestly. Repeat.</p><h2>365 days. Endless opportunities to help.</h2><div><BackendLink enabled={backendAvailable} href="/missions" className="button">Explore missions</BackendLink><BackendLink enabled={backendAvailable} href="/apply" className="button button-ghost">Submit a mission</BackendLink></div></section>
    <footer><div className="brand"><span className="brand-mark">M</span><span>MISSION <b>365</b></span></div><p>© 2026 Mission 365. A Kollective Hospitality Group platform.</p><div><BackendLink enabled={backendAvailable} href="/missions">Missions</BackendLink><BackendLink enabled={backendAvailable} href="/join">Join</BackendLink><BackendLink enabled={backendAvailable} href="/login">Sign in</BackendLink><BackendLink enabled={backendAvailable} href="/app">Dashboard</BackendLink><BackendLink enabled={backendAvailable} href="/apply">Apply</BackendLink><Link href="/download">Apps</Link></div></footer>
  </main>
}
